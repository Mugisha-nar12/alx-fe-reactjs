...
const Search = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const userResult = await searchUsers({ query: username, location });
      setUsers(userResult);
    } catch (err) {
      ...
      <div className="w-full max-w-2xl mx-auto">
        ...
        {users.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {users.map((user) => (
              <div key={user.id} className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
                <a href={user.html_url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center">
                  <img className="w-24 h-24 rounded-full mb-4" src={user.avatar_url} alt={`${user.login}'s avatar`} />
                  <h2 className="text-xl font-bold dark:text-white text-center">{user.login}</h2>
                  {user.location && <p className="text-gray-500 dark:text-gray-400 text-center">{user.location}</p>}
                </a>
              </div>
            ))}"
          </div>
